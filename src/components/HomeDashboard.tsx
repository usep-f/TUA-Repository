...
export const HomeDashboard: React.FC<HomeDashboardProps> = ({
  ...
  return (
    <div className="space-y-12 pb-16">
      {/* Hero Section */}
      <section className="relative overflow-hidden rounded-3xl glass-dark text-white p-6 sm:p-10 lg:p-12 border border-white/15 shadow-2xl">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center space-y-6">
          ...
          {/* Repository Pipeline Flow (Verbatim from PDF: Student scholarly work -> Repository -> Search -> Read -> Learn -> Cite) */}
          <div className="p-3.5 sm:p-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15 max-w-3xl mx-auto shadow-inner">
            <div className="text-[11px] font-bold uppercase tracking-widest text-amber-400 mb-2">
              Our Academic Workflow
            </div>
            <div className="flex flex-wrap items-center justify-center gap-2 text-xs font-semibold text-slate-200">
              <span className="px-2.5 py-1 rounded-lg bg-blue-950/80 text-amber-300 border border-blue-700/60 backdrop-blur-xs">
                Student Scholarly Work
              </span>
              <span className="text-amber-400 font-bold">→</span>
              <span className="px-2.5 py-1 rounded-lg bg-blue-950/80 text-slate-200 border border-blue-700/60 backdrop-blur-xs">
                Repository
              </span>
              <span className="text-amber-400 font-bold">→</span>
              <span className="px-2.5 py-1 rounded-lg bg-blue-950/80 text-slate-200 border border-blue-700/60 backdrop-blur-xs">
              ...
              <span className="text-amber-400 font-bold">→</span>
              <span className="px-3 py-1 rounded-lg bg-amber-400 text-blue-950 font-bold shadow-xs">
                Cite in APA 7th
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleSearchSubmit} className="max-w-2xl mx-auto relative pt-2">
            <div className="relative flex items-center shadow-2xl rounded-2xl">
              ...
              <button
                type="submit"
                className="absolute right-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-blue-950 font-bold text-sm transition-all shadow-md active:scale-95"
              >
                Search
              </button>
            </div>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
              ...
              {sampleKeywords.map((kw, i) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-full text-xs font-semibold bg-white/10 hover:bg-amber-400 hover:text-blue-950 text-amber-300 border border-white/20 backdrop-blur-xs transition-all"
                >
                  {kw}
                </button>
              ))}
          ...
          <div className="pt-6 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-3xl mx-auto border-t border-white/10 text-center">
            <div className="p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15">
              ...
              <div className="text-xs text-slate-300 font-medium">BSN Peer Reviewed</div>
            </div>
            <div className="p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15">
              <div className="text-2xl font-extrabold text-white">{works.length}+</div>
              <div className="text-xs text-slate-300 font-medium">Archived Works</div>
            </div>
            <div className="p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15">
              <div className="text-2xl font-extrabold text-amber-400">APA 7th</div>
              <div className="text-xs text-slate-300 font-medium">1-Click Citation</div>
            </div>
            <div className="p-3 rounded-2xl bg-white/10 backdrop-blur-md border border-white/15">
              <div className="text-2xl font-extrabold text-white">2nd Flr</div>
              <div className="text-xs text-slate-300 font-medium">Physical Reserve Desk</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">Follett Destiny Integrated</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {announcements.map((ann) => (
            <div
              key={ann.id}
              className="p-5 rounded-2xl glass-panel hover:border-amber-400/80 transition-all flex flex-col justify-between"
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${ann.badgeColor}`}>
                    {ann.category}
                  </span>
      ...
      <section className="rounded-none p-6 sm:p-8 lg:p-10 bg-[#081636] border border-blue-900/70 shadow-2xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className="text-xs font-bold text-amber-300 hover:text-amber-200 flex items-center gap-1.5 self-start sm:self-auto px-3.5 py-2 rounded-xl bg-blue-950/80 border border-blue-800/80 hover:border-amber-400/60 transition-colors"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {REPOSITORY_CURATED_COLLECTIONS.map((col) => (
            <div
              key={col.id}
              className="bg-white/95 rounded-2xl overflow-hidden flex flex-col justify-between group border border-white/20 shadow-md hover:shadow-xl hover:border-amber-400 transition-all"
            >
              <div className={`p-4 bg-gradient-to-r ${col.color} text-white space-y-1`}>
                <div className="flex items-center justify-between">
              ...
              <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                ...
                <div className="space-y-1 pt-2 border-t border-slate-200/60">
                  <span className="text-[11px] font-bold text-slate-700 block">Sub-specialties:</span>
                  <div className="flex flex-wrap gap-1">
                    {col.subcategories.map((sub, i) => (
                      <button
                        ...
                        className="text-[11px] px-2 py-0.5 rounded-md bg-slate-100 hover:bg-blue-950 hover:text-amber-300 text-slate-700 font-medium border border-slate-200/80 transition-colors"
                      >
                        {sub.name}
                      </button>
                    ))}
                ...
                <button
                  ...
                  className="w-full mt-2 py-2 rounded-xl bg-slate-50 group-hover:bg-amber-400 text-blue-950 text-xs font-bold transition-colors flex items-center justify-center gap-1 border border-slate-200 group-hover:border-amber-400 shadow-xs"
                >
      ...
      <section className="glass-dark rounded-none p-6 sm:p-8 text-white border border-white/15 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-amber-400 text-blue-950 text-xs font-bold mb-2">
              <GraduationCap className="w-4 h-4" />
              BSN Grand Case Presentation Archive
            </div>
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-blue-950 font-bold text-xs transition-colors shrink-0 flex items-center justify-center gap-2 shadow-md"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {recentGrandCases.map((work) => (
            <div
              ...
              className="p-4 rounded-xl bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/15 hover:border-amber-400 cursor-pointer transition-all flex flex-col justify-between group"
            >
              <div>
                <span className="text-[10px] font-bold text-amber-300 bg-amber-400/20 px-2 py-0.5 rounded border border-amber-400/30">
                  {work.batch} • {work.condition}
                </span>
                <h3 className="text-xs sm:text-sm font-bold text-white group-hover:text-amber-300 transition-colors mt-2 line-clamp-2">
                  {work.title}
                </h3>
              ...
              <div className="mt-3 pt-2 border-t border-white/10 flex items-center justify-between text-[11px] text-slate-400">
                <span className="font-mono text-amber-400/90">{work.physicalLibrary.callNumber}</span>
